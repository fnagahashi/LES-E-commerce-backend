...
export default class ValidationAvailabilityRoom implements IStrategy<Reservation> {
    constructor(private readonly reservationDAO: any) {}

    async executar(reservation: Reservation): Promise<string | undefined> {
        if (!reservation.room || !reservation.dateStart || !reservation.dateEnd) {
        ...
        try {
            ...
            const reservasOcupantes = reservasConflitantes.filter(r => 
                r.id !== reservation.id && 
                this.isQuartoOcupado(r.status)
            );
            ...
            return "Erro ao verificar disponibilidade do quarto";
        }
    }
