...
export default class ValidationRequiredAddressFields
  implements IStrategy<Address>
{
  async executar(address: Address): Promise<string | undefined> {
    const errors: string[] = [];

    if (!address.typeResidence?.trim())
      errors.push("Tipo de Residência é obrigatório");
    if (!address.typeStreet?.trim()) errors.push("Tipo de rua é obrigatório");
    ...
    if (!address.country?.trim()) errors.push("País é obrigatório");
