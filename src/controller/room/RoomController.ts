...
export class RoomController {
    ...
    public async buscarTodos(req: Request, res: Response): Promise<void> {
        try {
            // Usar 'findAll' que é o nome da operação no DAO
            // Passar objeto vazio castado como Room para evitar erro de tipos
            const rooms = await this.facade.list({} as Room, 'findAll');
    ...
    public async buscarPorRoomCode(req: Request, res: Response): Promise<void> {
        try {
            ...
            const rooms = await this.facade.list({ roomCode } as Room, 'findByFilters');
    ...
    public async buscarPorTipo(req: Request, res: Response): Promise<void> {
        try {
            ...
            const rooms = await this.facade.list({ type: type as RoomType } as Room, 'findByFilters');
    ...
    public async atualizarPreco(req: Request, res: Response): Promise<void> {
        try {
            ...
            const rooms = await this.facade.list({ id } as Room, 'findById') as Room[];
    ...
    private calcularCapacidadeMedia(rooms: Room[]): number {
        ...
        const totalCapacidade = rooms.reduce((sum, room) => sum + room.qntdAdultos + room.qntdCriancas, 0);
