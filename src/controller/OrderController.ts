...
export class OrderController {
  ...
  async reprovedExchange(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const order = await this.facade.findById("Order", id);
      ...
      const result = await this.facade.reprovedExchange(order as Order);

      res.status(200).json(result);
    } catch (error: any) {
      console.error("❌ Erro ao reprovar troca do Pedido:", error);
      res.status(400).json({
        success: false,
        error: error.message,
      });
