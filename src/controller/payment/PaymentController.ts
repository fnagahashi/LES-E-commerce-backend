...
export class PaymentController {
    ...
    public async marcarComoPago(req: Request, res: Response): Promise<void> {
        try {
            ...
            const payments = await this.facade.list({ id } as Payment, 'findById') as Payment[];
            ...
            if (paymentAtual.status === 'approved' || paymentAtual.status === 'confirmed') {
                res.status(400).json({
                    success: false,
            ...
            if (paymentAtual.status === 'refunded') {
                res.status(400).json({
                    success: false,
                    error: "Pagamento estornado não pode ser aprovado"
                });
                return;
            }
    ...
    public async estornarPagamento(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (paymentAtual.status !== 'approved' && paymentAtual.status !== 'confirmed') {
                res.status(400).json({
                    success: false,
                    error: "Somente pagamentos aprovados/confirmados podem ser estornados"
                });
                return;
            }
    ...
    public async buscarPorFiltro(req: Request, res: Response): Promise<void> {
        try {
            ...
            const payments = await this.facade.list(filters as Payment, 'findByFilters') as Payment[];
    ...
    public async getEstatisticas(req: Request, res: Response): Promise<void> {
        try {
            ...
            const stats = {
                ...
                estornos: payments.filter(p => p.status === 'refunded').length,
    ...
    private calcularTaxaAprovacao(payments: Payment[]): number {
        ...
        const aprovados = payments.filter(p => 
            p.status === 'approved' || p.status === 'confirmed'
        ).length;
